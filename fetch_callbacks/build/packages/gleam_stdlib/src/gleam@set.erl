...
-opaque set(EEY) :: {set, gleam@dict:dict(EEY, list(nil))}.
...
-spec contains(set(EFJ), EFJ) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(EFL), EFL) -> set(EFL).
delete(Set, Member) ->
...
-spec fold(set(EFU), EFW, fun((EFW, EFU) -> EFW)) -> EFW.
...
-spec filter(set(EFX), fun((EFX) -> boolean())) -> set(EFX).
...
-spec drop(set(EGE), list(EGE)) -> set(EGE).
drop(Set, Disallowed) ->
...
-spec intersection(set(EGV), set(EGV)) -> set(EGV).
...
-spec difference(set(EGZ), set(EGZ)) -> set(EGZ).
...
-spec is_subset(set(EHD), set(EHD)) -> boolean().
...
-spec is_disjoint(set(EHG), set(EHG)) -> boolean().
...
-spec each(set(EHN), fun((EHN) -> any())) -> nil.
each(Set, Fun) ->
    fold(
        Set,
        nil,
...
-spec insert(set(EFG), EFG) -> set(EFG).
insert(Set, Member) ->
...
-spec from_list(list(EFR)) -> set(EFR).
from_list(Members) ->
...
-spec map(set(EGA), fun((EGA) -> EGC)) -> set(EGC).
map(Set, Fun) ->
...
-spec union(set(EGM), set(EGM)) -> set(EGM).
union(First, Second) ->
    {Larger, Smaller} = order(First, Second),
...
-spec symmetric_difference(set(EHJ), set(EHJ)) -> set(EHJ).
